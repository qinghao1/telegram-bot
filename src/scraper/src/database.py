...
from .queries import (
    breakfast_insert_query,
    breakfast_insert_template,
    dinner_insert_query,
    dinner_insert_template,
)
...
def breakfast_insert_many(ops):
    conn = connect()
    cur = conn.cursor()
    execute_values(cur, breakfast_insert_query, ops, breakfast_insert_template)
    conn.commit()
    cur.close()
...
def dinner_insert_many(ops):
    conn = connect()
    cur = conn.cursor()
    execute_values(cur, dinner_insert_query, ops, dinner_insert_template)
    conn.commit()
    cur.close()
